...
| 方式 | 适用 | 涨跌家数可靠性 | HTTPS（Android 安装） |
| --- | --- | --- | --- |
| ① 本机 `server.js` | 电脑本机 / 局域网 | ✅ 服务端 `/api/em` 代理，最稳 | ❌ 需自有域名+证书 |
| ② 云端静态托管（CloudStudio 等） | 公网访问 | ⚠️ 浏览器 CORS 代理兜底 | ✅ 平台自带 HTTPS |
| ③ 云端运行 `server.js`（Render/VPS） | 公网 + 最稳 | ✅ 服务端代理 | ✅ 平台自带 HTTPS |

> ⚠️ **重要**：CloudStudio 等静态托管平台**无法运行 Node / 后端服务**（平台限制），
> 所以「云端静态版（②）」永远拿不到服务端代理，涨跌家数只能靠浏览器侧公共 CORS 代理兜底，偶发 `—`。
> 要做到「涨跌家数最稳」，必须把云端升级为**真正运行 `server.js` 的完整代理版（③）** ——
> 完整步骤见 👉 **[DEPLOY-RENDER.md](DEPLOY-RENDER.md)**（Render 免费版 2 分钟 / VPS Docker）。
...
| `render.yaml` / `Dockerfile` | Render / Docker 部署配置 |
